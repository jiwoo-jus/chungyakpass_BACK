...
public class VerificationOfSpecialKookminNewlyMarriedServiceImpl implements VerificationOfSpecialKookminNewlyMarriedService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public VerificationOfSpecialKookminNewlyMarriedResponseDto updateSpecialKookminNewlyMarriedVerification(Long id, VerificationOfSpecialKookminNewlyMarriedUpdateDto verificationOfSpecialKookminNewlyMarriedUpdateDto) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean secondChungyak(User user) { //2순위청약신청여부
        List<HouseMember> houseMemberListUser = houseMemberRepository.findAllByHouse(user.getHouseMember().getHouse()); //신청자의 세대구성원 가져오기
