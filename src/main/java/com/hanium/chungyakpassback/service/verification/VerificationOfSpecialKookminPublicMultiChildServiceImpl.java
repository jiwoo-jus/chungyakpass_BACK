...
import com.hanium.chungyakpassback.repository.verification.VerificationOfSpecialKookminMultiChildRepository;
...
@Service
public class VerificationOfSpecialKookminPublicMultiChildServiceImpl implements VerificationOfSpecialKookminPublicMultiChildService {
    ...
    @Override //특별다자녀국민조회
    public List<VerificationOfSpecialKookminPublicMultiChildResponseDto> readSpecialKookminMultiChildVerifications() {
        ...
        return verificationOfSpecialKookminPublicMultiChildResponseDtos;
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public VerificationOfSpecialKookminPublicMultiChildResponseDto createSpecialKookminPublicMultiChildVerification(VerificationOfSpecialKookminPublicMultiChildDto verificationOfSpecialKookminPublicMultiChildDto) {
        User user = userRepository.findOneWithAuthoritiesByEmail(SecurityUtil.getCurrentEmail().get()).get();
        HouseMember houseMember = user.getHouseMember();
        AptInfo aptInfo = aptInfoRepository.findById(verificationOfSpecialKookminPublicMultiChildDto.getNotificationNumber()).orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_APT));
        AptInfoTarget aptInfoTarget = aptInfoTargetRepository.findByResidentialAreaAndAptInfo(verificationOfSpecialKookminPublicMultiChildDto.getResidentialArea(), aptInfo).orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_APT));
        ...
        VerificationOfSpecialKookminMultiChild verificationOfSpecialKookminMultiChild = new VerificationOfSpecialKookminMultiChild(user, americanAge, meetLivingInSurroundAreaTf, accountTf, meetMonthlyAverageIncomeTf, meetPropertyTf, meetHomelessHouseholdMembersTf, meetAllHouseMemberRewinningRestrictionTf, calcMinorChildren, householderTf, meetBankbookJoinPeriodTf, meetNumberOfPaymentsTf, isRestrictedAreaTf, aptInfo, aptInfoTarget);
        verificationOfSpecialKookminMultiChildRepository.save(verificationOfSpecialKookminMultiChild);
        return new VerificationOfSpecialKookminPublicMultiChildResponseDto(verificationOfSpecialKookminMultiChild);
    }
